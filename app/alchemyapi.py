...
if __name__ == '__main__':
    """
    Writes the API key to alchemyapi_key.txt file. It will create the file if it doesn't exist.
    ...
    if len(sys.argv) == 2 and sys.argv[1]:
        if len(sys.argv[1]) == 40:
            ...
            f = open('alchemyapi_key.txt', 'w')
            f.write(sys.argv[1])
            f.close()
            print('Key: ' + sys.argv[1] + ' was written to alchemyapi_key.txt')
            print(
...
class AlchemyAPI:
    ...
    def __init__(self):
        ...
        It loads the API key from alchemyapi_key.txt and configures the endpoints.
        """

        import sys
        try:
            # Open the key file and read the key
            f = open("alchemyapi_key.txt", "r")
            key = f.read().strip()

            if key == '':
                # The key file should't be blank
                print(
                    'The alchemyapi_key.txt file appears to be blank, please run: python alchemyapi.py YOUR_KEY_HERE')
                print(
                ...
                print(
                    'It appears that the key in alchemyapi_key.txt is invalid. Please make sure the file only includes the API key, and it is the correct one.')
                sys.exit(0)
            else:
                # setup the key
                self.apikey = key
            ...
            open('alchemyapi_key.txt', 'a').close()
            sys.exit(0)
